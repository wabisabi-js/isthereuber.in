...
export default ({ isSearching }) => {
  ...
  const [mouthIsOpen, setMouthIsOpen] = useState(true)
  const [writing, setWriting] = useState(true)

  useEffect(() => {
    setInterval(() => {
      return setEyeIsOpen(false)
    }, 4000)
    setTimeout(() => {
      return setEyeIsOpen(true)
    }, 250)
    setInterval(() => {
      return setMouthIsOpen(false);
    }, 5000)
    setTimeout(() => {
      return setMouthIsOpen(true);
    }, 250)

  }, [eyeIsOpen, mouthIsOpen])
  return (
    <Planet
